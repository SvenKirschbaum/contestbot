...
public class ContestBotTest {
    ...
    public void testLoadModules() {
        ...
        Events[] types = TestModule.class.getAnnotation(Model.EventTypes.class).value();
        EnumSet<Events> set = EnumSet.noneOf(Events.class);
        set.addAll(Arrays.asList(types));
        MessageParser.registerObserver(TestModule.class.isAnnotationPresent(Model.CallFirst.class), set,
                new TestModule());
